...
fixed_df <- data_frame(type='discrete', mu=.5, lb=-4, ub=Inf, minconc=0.001, maxconc=30, ndoses=7, nreps=1, sd_prop=0.3)
#varying_df <- crossing(sd=c(.4, 1), n=c(50, 200, 800), prop=c(0.05,0.1,0.2), beta=-c(.5, 1), sd_add=0.5) %>%
  varying_df <- crossing(sd=c(1), n=c(10, 20, 40), prop=c(.5), beta=-c(.5, 1, 2), sd_add=c(0.15,.45, 1)) %>%
  dplyr::mutate(sim_group=row_number())


complete_df <- crossing(fixed_df, varying_df, sim_rep=c(1:50)) %>%
