...
```{r}
pIC50_data2 <- sim_cell_lines(n=100, type='d', prop=0.2, mu=1, sd=0.1, beta=1)
...
```{r}
pIC50_exp_data <- sim_cell_lines(n=500, type='c', mu=1, sd=0.1, prho=sqrt(0.6))
pIC50_exp_data
```
...
```{r}
nfit <- minpack.lm::nlsLM(resp~1-conc/(exp(ic50)+conc),
                    data=ex_dr2,
                    start = c(ic50=1))
nfit
```
...
The pIC50 values obtained can be extracted using the `tidy` function from the `broom` package:

```{r}
nls_pIC50s <- nls_results %>%
  dplyr::mutate(res=purrr::map(fit, broom::tidy)) %>%
  dplyr::select(-dr_data,-fit) %>%
  tidyr::unnest() %>%
  dplyr::rename(nls_pIC50=estimate) 
nls_pIC50s
```
...
```{r}
ggplot(nls_pIC50s, aes(x=pIC50, y=nls_pIC50)) +
  geom_point() +
  theme_bw() +
  facet_wrap(~sim_no) +
  geom_abline(slope=1, intercept=0, colour='blue')
...
The nls followed by lm procedure does a good job of estimating the beta coefficient, although more simulations would be necessary to confirm this.
