...
```{r}
ex_dr1 <- sim_dose_response(pIC50=0, lb=0.001, ub=30, 
                            ndoses=10, nreps=1, sd_prop=0, sd_add=0)
ex_dr1
```
Here we specify the pIC50 value, the lowest concentration tested `lb`, the highest concentration tested `ub`, the number of doses `ndoses`, the number of replicates per dose `nreps`, and the additive and proportional variance `sd_prop` and `sd_add`.  Since we specified zero variance, we get the perfect dose response curve with an IC50 of 1 (pIC50=0):
...
```{r}
ex_dr2 <- sim_dose_response(pIC50=0, lb=0.001, ub=30, 
