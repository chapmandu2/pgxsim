...
However, since each simulateion is independent of the others, parallelisation is the obvious way forward.  In the example below, the BatchJobs package is used since this allows the same code to be run in a number of different computing environments, from multicore processing on a workstation or laptop, to 1000s of cores across a cluster or cloud environment.
...
### Configure BatchJobs environment

The code below configures BatchJobs to run on a MacOS or Linux computer.  Alternative `makeClusterFunctions` options are available for common job schedulers.
...
Next the BatchJobs registry is created, being sure to define the packages that are needed:

```{r, eval=FALSE}
reg <- makeRegistry(id="BatchJobs",
                    packages=c('pgxsim', 'tidyverse'),
                    work.dir = bjwork_dir,
...
Now we see more clearly what we would expect, that all methods perform better when the effect size is large and the proportion of cell lines with the genetic mutation is high.  The `nls_lm` method seems weaker than the mixed-effects based methods.

# Conclusion

This vignette has described the principles required to run large scale pharmacogenomic simulation studies in a parallelised computing environment. 
